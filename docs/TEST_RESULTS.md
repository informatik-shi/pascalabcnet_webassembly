...
`tests/browser/lightpt.html`: **PASS**. Один скрытый `Tasks.pas` проверил два разных набора входных данных через эталонный `ReferenceSolve`; заведомо неверное решение получило `BadSolution`, ожидаемое значение не попало в результат, а после проверки стандартный вывод был восстановлен.
